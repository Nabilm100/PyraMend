  ...
  try {
    ...
    if (water.remainingWater === 0) {
      return res.json({
        ...
        waterConsumed: water.waterConsumed,
      });
    }

    res.json({
      message: `Water consumption tracked successfully. Remaining target: ${water.waterConsumed}ml = ${remainingLiters}.${remainingMilliliters}L`,
      waterConsumed: water.waterConsumed,
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
